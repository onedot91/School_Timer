...
- Student-facing UI is Chromebook-first and does not require phone-specific proportions or mobile-only rearrangements unless the user explicitly requests them. **Use exactly `1280×800` (`16:10`) at `100%` preview scale as the primary design and code-improvement viewport.**
  - Before changing layout, spacing, sizing, or first-screen fit, confirm that the browser device toolbar shows `1280 × 800` and `100%` and that the page reports `window.innerWidth === 1280` and `window.innerHeight === 800`. A scaled result such as `1075×672` is not primary QA evidence even if it was produced from a nominal 1280×800 host viewport.
  - Never tune CSS from, approve, or mark a UI task complete using an `84%` or otherwise scaled preview. Never compensate by substituting larger viewport dimensions such as `1523×952`, and never treat a scaled screenshot, DOM measurement, or reviewer pass as equivalent to the primary viewport.
  - Run the exact `1280×800`·`100%` check again after the final layout-affecting edit. The task remains incomplete until the latest implementation is observed without clipping, overlap, unintended scrolling, or first-screen overflow at that exact viewport.
  - If browser automation cannot set or prove both the toolbar scale and CSS viewport, stop primary visual QA immediately, leave the visual-verification step incomplete, report the limitation, and ask the user to select `100%`. Do not continue dimension-driven implementation from the scaled view and do not claim the UI fix is complete.
  - Only after the exact primary viewport passes, visually verify 1024px and 1366px widths as secondary responsive checks. Preserve text zoom, keyboard access, and overflow safety at all three widths.
