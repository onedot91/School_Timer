...
- Student-facing UI is Chromebook-first and does not require phone-specific proportions or mobile-only rearrangements unless the user explicitly requests them. **Use exactly `1280×800` (`16:10`) at `100%` preview scale as the primary design and code-improvement viewport:** the browser device toolbar itself must show `1280 × 800` and `100%` during primary visual QA. Never compensate for a non-100% preview scale by substituting larger viewport dimensions (for example, `1523×952` at `84%`). If browser automation cannot change the host preview scale to `100%`, stop primary visual QA, clearly report the limitation, and ask the user to select `100%`; do not continue QA using a compensated viewport or claim exact-size verification. Make layout, spacing, sizing, and first-screen fit decisions against the exact primary viewport before adapting elsewhere. Then visually verify 1024px and 1366px widths as secondary responsive checks; preserve text zoom, keyboard access, and overflow safety at all three widths.
