...
- Student-facing UI is Chromebook-first and does not require phone-specific proportions or mobile-only rearrangements unless the user explicitly requests them. **Use exactly `1280×800` (`16:10`) as the primary and required design/code-improvement viewport.**
  - Before changing layout, spacing, sizing, or first-screen fit, confirm that the browser viewport is `1280 × 800` (`window.innerWidth === 1280`, `window.innerHeight === 800`). Preview zoom or scale percentage is not a pass/fail requirement.
  - Run the exact `1280×800` check again after the final layout-affecting edit. The task remains incomplete until the latest implementation is observed without clipping, overlap, unintended scrolling, or first-screen overflow at that viewport.
  - Do not require additional viewport-width checks such as 1024px or 1366px unless the user explicitly requests them. Preserve text zoom, keyboard access, and basic overflow safety.
