...
| failure story type | `clamp(1.2rem, 1.8vw, 1.5rem)` | 카드 중앙에서 실패 내용을 빠르게 읽는 굵은 본문 |
| failure lesson type | `clamp(1.05rem, 1.5vw, 1.25rem)` | 다음 시도를 두 줄 안에서 읽는 민트 보조 본문 |
...
- The failure exhibition uses only persisted `studentLife.failureStories` records from the shared Supabase snapshot or localStorage fallback; it never injects generated examples or preview-only stories into the student feed. The page uses one warm off-white exhibition wall as the gallery owner. Two horizontal wire lines cross the wall and each visible story hangs from one centered sand-colored ring clip. At the authoritative `1280×800` viewport, the wall presents six portrait papers in a `3 × 2` grid. Each paper uses a white sheet with a restrained tactile shadow, one softly colored writing field that cycles mint, blush, and lavender, and a slight deterministic rotation that never tilts text enough to reduce legibility. Every story, including the current student's story, participates in the same moving six-paper window without duplication. No author name, number, visible `익명` label, ranking, or public cheer count appears.
