...
- Student mode uses four page-like views: `overview`, `emotions`, `missions`, and `store`. The overview is the default hub and contains one reserved 16:9 character-and-background artwork canvas, one compact grouped identity-and-balance summary, one dedicated daily-emotion card, and exactly two dominant destination cards: `고마 벌기` for missions and `고마 쓰기` for the auction and donation. The student number appears at the leading edge of the balance summary and never overlays the artwork. Mission and donation summaries appear only inside their matching destination card; mission details never appear in the store, and auction items never appear in the overview.
