...
| `--classword-paper` / `--classword-paper-filled` | `#fffdf7` / `#e8f4ed` | 비어 있는 초성 종이와 제출된 초성 종이 |
| `--classword-line` / `--classword-line-strong` | `#c8d8cf` / `#78a893` | 낱말판 칸, 선택, 입력·완료 상태의 경계 |
...
| `--classword-quiz-initial-size` | `6rem` | 줄어든 보드에서 돌려받은 높이를 퀴즈의 핵심 단서에 사용하도록 만드는 정사각형 전체 초성 힌트 크기 |
| Classword quiz material | existing `--classword-prompt-soft`, `--classword-sky`, `--classword-accent`, and `--classword-paper` | 초성 힌트, 뜻·예문 종이, 정답 입력과 완료 상태를 낱말판과 같은 재료로 연결하며 새 장식색을 만들지 않음 |
...
- Classword initial cells use the existing button press token and crossfade only their border, paper tone, and content state. A successful submission settles with one short scale-and-opacity response; completing all 14 cells emits a single `--student-motion-celebrate` transform/opacity particle sequence. Keyboard entry and `prefers-reduced-motion` replace both effects with immediate state and a static completion outline. The word quiz uses only the shared press response and an immediate text/icon correctness state; an incorrect answer never shakes, flashes, or reveals additional letters.
...
- The student shop is a Chromebook-first three-part destination: `물품`, `캐릭터 뽑기`, and `집`. Teacher-authored fixed-price goods and per-student purchase counts share the existing settings JSON. Character draws cost 100 고마 and immediately become the active home-canvas character. The house workshop stays locked until `집 고치기`; its supplied carpenter-elephant guide is the dominant locked-state artwork, and the character's green uniform defines the panel, borders, and primary actions before and after repair. The active `집` tab uses the existing wood-brown shop accent so it does not merge visually with the green workshop. Market houses cost 100 고마 and a custom design coupon costs 150 고마. After repair, double-clicking or double-tapping the home-canvas house opens a true modal containing the repaired default house, owned market houses, and the completed custom house; keyboard users open the same modal with Enter or Space. Avoid mobile-only rearrangements for this surface; validate at 1024, 1280, and 1366px widths.
- Teacher shop settings mirror the student's three-part navigation as `물품`, `고마 스킨 뽑기`, and `집`. Item registration and purchase history stay together under `물품`, including `내 집 만들기` after the student buys its 150고마 coupon. The complete skin catalog stays under `고마 스킨 뽑기`, and the purchasable house catalog stays under `집`; switching tabs never changes the existing shop or student economy persistence shapes.
...
- The Classword board is a 14-cell paper grid: seven columns by two rows at `1280×800`, then four, three, two, or one column as the effective width narrows. At the reference viewport the board is capped at `--classword-board-height`; this keeps both rows compact, moves profile and edit actions closer to their words, and gives the bottom quiz the remaining task height instead of letting the board consume it. Its supplied reference is translated into the existing School Timer material language: the topic becomes one large sentence above the board with a lime-to-sky underline beneath only the topic word, the board is one broad white paper surface, and empty cells use a quiet diagonal paper hatch, dashed boundary, centered inline initial/tense-alias pair, and a lime circular `+`. Filled, owned, selected, editing, confirmation, and deletion states replace the hatch with solid semantic paper and retain their explicit text/icon signals. The pre-save confirmation removes its explanatory question and uses two 48px round symbol-only actions: check to save and X to revise; both retain explicit accessible names. Every submitted answer uses the same `clamp(1.55rem, 2.2vw, 1.9rem)` type scale regardless of word length; a long unbroken word wraps inside its own card instead of shrinking relative to neighboring answers. Empty cells are enabled only while the student has no entry that day; another student's filled cell is read-only; the student's own cell exposes edit and delete. Focus, ownership, occupied, saving, error, and complete states always use text or icon shape in addition to color. The board pane owns overflow at large text sizes so the document shell remains stable. No helper-character rail is reserved: all seven columns consume the full paper width with equal inline padding, while save and error feedback remains a compact non-character status layer that does not reduce card width.
- The daily Classword quiz is one compact section owner: the supplied `보너스 문제` illustration replaces a visible text heading while a screen-reader heading remains, followed by one `--classword-quiz-initial-size` square initial tile, one solid meaning/two-example paper, and one answer form. Submission reports correct or incorrect immediately through Korean text and an icon in addition to color. A correct student/date/question tuple is idempotent, survives reload, disables both the input and confirmation action, and never displays the answer after completion. At `1280×800`, the board keeps its fixed content budget while the quiz uses only its intrinsic height and leaves deliberate bottom padding; this prevents both decorative empty space inside the section and a card stuck to the viewport floor. At text zoom or narrow effective widths the same content pane owns vertical scrolling.
