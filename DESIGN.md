...
| `--student-mail-list-ratio` / `--student-mail-reader-ratio` | `34fr` / `66fr` | Chromebook mailbox split between the bounded envelope list and reading stage |
...
| `--student-letter-rule` | `2rem` | shared ruled-paper cadence for the letter background, body line-height, and signature rhythm |
| `--student-letter-rule-offset` | `3px`; `5px` above `1100px` | responsive ruled-paper baseline offset that follows the body font scale and places body and signature glyphs directly on each horizontal rule |
| `--student-letter-signature-offset` | `0px`; `2px` above `1100px` | signature-only baseline correction at the larger Chromebook type scale |
...
- The student bookstore reuses the shared task header and presents published writing cards on one horizontal wood shelf. Cards are real buttons with a title and a preview drawn from the body; opening one uses the shared blocking-dialog material and focus trap. The separate bookshelf view reuses the existing book-stack form and data unchanged. The teacher bookstore editor accepts only a title and body, and keeps explicit up/down ordering controls plus a published-state toggle.
...
- Mail and reading records stay in one compact `studentLife` field inside the existing shared settings snapshot. The mailbox keeps received letters and supports student-to-student or teacher-to-student composition; opening an unread letter records its read time. The library records a title and page count and renders the student's latest books as a visual stack. Each book spine maps the current stack's minimum and maximum page counts linearly into a bounded `27px–45px` visual range, so page differences stay visible without one unusually long book dominating the canvas. The stack top separately shows the summed physical estimate using `1쪽 = 0.005cm`, with no per-book cover correction. An oversized stack scrolls inside the bookshelf. These views reuse the existing Supabase updater and localStorage fallback without another table, dependency, or polling loop.
- The student mailbox is a `34:66` list-detail workspace at `1280×800`. The left pane alone owns list scrolling and renders real DOM envelope buttons: unread mail remains closed with a text label and wax seal, read mail shows an opened flap, and the selected envelope advances slightly without changing surrounding geometry. Sender category is encoded by a labelled stamp variant: bank/system green, teacher yellow, and student mail alternating pink or blue. The right stage contains either one content-height letter, a compact Goma empty state, or the compose form; it never reserves a full-height blank paper. Received, sent, and compose tabs reuse the existing `studentLife.letters` array, read timestamp mutation, send path, and Supabase/localStorage fallback without changing stored records.
...
- Student missions use two explicit groups: the built-in `감정 구슬 넣기`, `스도쿠`, and `3자리 숫자야구` tasks plus teacher-configured items appear under `일일 미션`, and synced rewards appear under `주간 미션`. The emotion task opens the internal picker, gives `5 고마` only on the first saved emotion for that Korean local date, and presents completion after the reward is recorded. Number baseball assigns one deterministic three-distinct-digit answer per student and Korean date, keeps up to nine attempts in the shared-settings snapshot with localStorage fallback, and pays exactly once: attempts 1–5 give `15 고마`, 6–7 give `10 고마`, 8–9 give `5 고마`, and exhaustion gives none. Number-baseball history stays chronological with `1회` at the top, always reserves all nine attempt rows with dashed `입력 전` placeholders, and separates the three reward bands with quiet tonal surfaces and compact reward labels rather than repeated range headings or thick boundary lines. The next available attempt uses a restrained current-step surface and `aria-current`, while later slots remain quieter. At the 1280×800 reference viewport those reward groups form three horizontal bands beside the play controls, so all nine reserved attempts remain visible without an inner scrollbar and the attempt, reward, and result type stays at student-readable size. Result badges show only positive `S` and `B` counts; `OUT` appears only when all three digits are absent from the answer and never carries a numeric prefix. The bold S/B/OUT code is the badge's primary text, while the Korean meaning remains readable supporting text; the existing solid/double/dashed borders preserve distinction beyond color. Its mission card shows `+5 / +10 / +15 고마` and distinguishes `진행 전`, `진행 중`, `완료`, and `기회 소진`. The weekly `신문에 개인 질문하기` mission gives `10 고마`; the two Classword weekly missions each give `5 고마`. Each mission card exposes a bordered status chip, prominent reward pill, title/short next-step copy, and one full-width action. Sudoku distinguishes `진행 전`, `진행 중`, and `완료` before the action label is read. The mission page may show one progress/reward summary, while the overview stays free of detailed mission metrics. Only verified existing service destinations may become links; missions without a destination contract show a disabled classroom action instead of an invented URL.
