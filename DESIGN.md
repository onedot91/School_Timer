...
| `--student-overview-balance-width` | `clamp(20rem, 29vw, 24rem)` | centered balance column width between the two overview destination cards |
...
- The overview destination cards mirror around the centered balance: `고마 벌기` places its left-pointing action at the outer-left edge and its title/icon toward the center, while `고마 쓰기` places its title/icon toward the center and its right-pointing action at the outer-right edge.
