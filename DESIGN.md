...
| `--student-character-stage-min-height` | `15rem` | expanded overview character stage |
| `--student-character-stage-image-size` | `15rem` | character artwork scale inside the expanded stage |
| `--student-balance-compact-height` | `4rem` | compact overview balance summary |
| `--student-emotion-summary-size` | `7.5rem` | prominent overview emotion artwork control |
...
- Student mode uses four page-like views: `overview`, `emotions`, `missions`, and `store`. The overview is the default hub and contains one reserved, empty character stage, one compact grouped balance summary, one dedicated daily-emotion card, and exactly two dominant destination cards. Mission and donation summaries appear only inside their matching destination card; mission details never appear in the store, and auction items never appear in the overview.
- `StudentHeader` on task pages contains only the overview return action and the current page title. Student identity and character are not repeated outside the overview. `StudentBalanceSummary` presents total, available, and reserved 고마 as one grouped unit instead of three competing cards; when no amount is reserved, the empty reservation state is omitted rather than rendered as a second card.
...
- The `내 기록` section of `StudentEmotionPage` uses a seven-column monthly calendar. A recorded date pairs its emotion orb with a restrained zone-colored cell surface, date color, and selected ring so the month can be scanned by color without losing the image cue. The adjacent sidebar separates the selected date detail from a compact monthly zone summary showing red, yellow, blue, and green counts and proportions. Month navigation changes both the calendar and summary without changing the stored history.
