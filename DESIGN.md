...
- The daily Classword quiz is one compact section owner: a visible section heading, one `--classword-quiz-initial-size` square initial tile, one solid meaning/example paper, and one answer form. Submission reports correct or incorrect immediately through Korean text and an icon in addition to color. A correct student/date/question tuple is idempotent, survives reload, disables both the input and confirmation action, and never displays the answer after completion. At `1280×800`, the board and quiz share the task content budget with the quiz visible at the bottom; the smaller board gives the clue, copy, and controls a larger third-grade-readable scale instead of leaving decorative empty space. At text zoom or narrow effective widths the same content pane owns vertical scrolling.
