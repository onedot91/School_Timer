...
| failure hanging grid | `3 × 2` at `1280×800` | 두 줄의 얇은 와이어에 한 줄당 세 장을 걸어 여섯 이야기를 한눈에 보여 주는 전시 구조 |
| failure paper clip | `2.25rem × .7rem` | 각 종이 중앙을 와이어에 고정하는 모래색 링 클립; 장식이며 포인터 이벤트를 받지 않음 |
...
`createdAt + id` identity, then keyed by `FailureStory.id`. The first six and the canonical latest six use
all six tones once. Relay movement, input-array reordering, body edits, and reloads must preserve each
story's tone, so no persisted color field or slot-based `nth-child` styling is allowed. At the Chromebook
...
- The failure exhibition uses only persisted `studentLife.failureStories` records from the shared Supabase snapshot or localStorage fallback; it never injects generated examples or preview-only stories into the student feed. The page uses one warm off-white exhibition wall as the gallery owner. Two horizontal wire lines cross the wall and each visible story hangs from one centered sand-colored ring clip. At the authoritative `1280×800` viewport, the wall presents six portrait papers in a `3 × 2` grid. Each paper uses a white sheet with a restrained tactile shadow and one of six softly colored writing fields: mint, blush, lavender, sky, butter, or aqua. Cards stay level with no decorative rotation. Every story, including the current student's story, participates in the same moving six-paper window without duplication. No author name, number, visible `익명` label, ranking, or public cheer count appears.
- Every hanging paper keeps the story as its visual focus and shows the failed attempt and next attempt as one two-part reading unit. The main sentence uses at least `1.15rem` and the next-attempt sentence uses at least `1rem` at `1280×800`; both preserve Korean word units and balance their two-line wraps so a short final word never hangs alone before truncation, and activating the paper reveals the complete text without changing the wire-row geometry. The anonymous animal profile remains visible in the sheet footer at no less than `2.75rem`. Classmates keep one clearly labelled, at least 44px-high `응원하기` control beside the profile. Its three text cheer choices open as an overlay inside the same paper, can be changed or cleared, never expose a public count, and close with Escape or an outside press. The current student's own paper shows the profile but no self-cheer action. The bookshelf action stays in the task header, and the circular writing action remains in the wall's lower-right utility cluster without covering a paper.
- When more than six failure stories exist, the gallery becomes a cyclic `링 페이퍼 릴레이`: one moving six-paper window contains all authors under the same ordering rule. The window advances by one story every 5.5 seconds while a complete contiguous six-paper window remains available; after the oldest complete window it returns to the newest complete window instead of mixing the list's two ends. This keeps all six visible tones distinct while preserving each story's tone and DOM identity. Entering and leaving papers translate along the wire direction for `900ms` without opacity, blur, or stagger; reduced motion swaps them immediately. Pointer hover, focus within, an expanded story, cheer selection, and composition pause the timer only while active. Arrow keys, wheel navigation, vertical swipe, and the compact previous/next control navigate the same cyclic window. Incoming stories never displace a paused reader, and one `새 이야기 N개` action offers an explicit jump to the latest set. The DOM contains only the current six-paper window rather than the full archive.
