...
- The student securities flow uses third-grade language and one question per screen. `내 투자` shows only invested 고마, current value, total result, and owned stocks; it does not repeat the full market. `종목 고르기` shows all four stocks in one `4×1` row at the 1280×800 Chromebook viewport, with ownership, one red-up/blue-down result, today's news, one price, and one action. Buying or selling always opens a short confirmation dialog and uses `개`, `사기`, `팔기`, and `고마` instead of specialist trading terms.
...
- 초등 3학년용 증권사는 `내 투자`와 별도 `종목 거래` 화면으로 나뉜다. 첫 화면은 보유 종목 수, 지금 수익, 지금 팔면 받는 고마를 먼저 보여주고, 보유 종목별 결과와 전체 네 종목의 당일 오르내림은 짧은 요약만 남긴다. 상승은 빨간 `▲ n 고마`, 하락은 파란 `▼ n 고마`, 보합은 중립 `－ 0 고마`로 표시하며 퍼센트를 쓰지 않는다. `종목 거래` 화면은 1280×800에서 네 종목을 한 행의 4열 표처럼 배치하고, 각 칸은 `종목명 → ▲/▼ 고마 → 등락 이유 → 가격·사기/팔기` 순서만 남긴다. 보유 중인 종목에만 `보유 1개` 배지를 표시하며 미보유 설명, 화면 안내문, `오늘 소식` 같은 중복 제목은 표시하지 않는다. 지난 기록이 있을 때만 짧은 `이전 기록` 제어로 이유를 다시 볼 수 있다. 자동 정산은 하지 않으며 기존 보유 기록은 그대로 수동 매도할 수 있다.
