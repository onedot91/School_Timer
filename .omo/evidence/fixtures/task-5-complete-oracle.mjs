...
const outputDirectory = path.join(root, '.omo/evidence/task-1-apple-ui-refresh-plan');
const outputPath = path.join(root, '.omo/evidence/task-5-apple-ui-refresh-plan/complete-current-oracle.json');
...
const result = {
  schemaVersion: 1,
  purpose: 'Complete current Apple UI visible-text and interaction oracle after Timer home refresh',
...
try {
  ...
  start(
    ...
    { DISABLE_HMR: '1', VITE_SUPABASE_URL: 'http://127.0.0.1:54329', VITE_SUPABASE_ANON_KEY: 'qa-only-fake-key' },
  );
  ...
  await page.getByRole('dialog', { name: '낙찰 애니메이션' }).getByText('2 / 2', { exact: false }).waitFor();
  ...
  await page.getByText('이번 주 경매를 마감할까요?', { exact: true }).waitFor();
  ...
  await page.locator('.settings-dialog .settings-header button').last().click();
  ...
  await context.addInitScript(() => {
    if (location.pathname.endsWith('/random-draw.html') && sessionStorage.getItem('task5-draw-initialized') !== '1') {
      localStorage.removeItem('school-random-draw-v1');
      sessionStorage.setItem('task5-draw-initialized', '1');
    }
  });
  ...
  const pristineBaseline = JSON.parse(await readFile(path.join(root, '.omo/evidence/task-1-apple-ui-refresh-plan/complete-visible-text-oracle.json'), 'utf8'));
  const repairedBaseline = JSON.parse(await readFile(path.join(root, '.omo/evidence/task-6-apple-ui-refresh-plan/complete/authoritative-todo6-baseline.json'), 'utf8'));
  const standaloneDrawBaseline = JSON.parse(await readFile(path.join(root, '.omo/evidence/task-7-apple-ui-refresh-plan/runtime.json'), 'utf8'));
  const repairedStateIds = new Set([
    'timer/question-submitted', 'timer/question-loading', 'timer/question-error', 'timer/question-empty',
    'timer/auction-award-confirmation', 'timer/auction-award-presentation', 'timer/auction-reset-confirmation',
    'timer/embedded-draw-rolling', 'timer/embedded-draw-winner', 'timer/embedded-draw-repeat',
  ]);
  const drawRuntimeRepairIds = new Set([
    'timer/embedded-draw-rolling', 'timer/embedded-draw-winner', 'timer/embedded-draw-repeat',
  ]);
  const standaloneDrawStateMap = new Map([
    ['random-draw/default', 'default'],
    ['random-draw/settings', 'settings'],
    ['random-draw/after-settings', 'after-settings'],
    ['random-draw/winner', 'winner'],
    ['random-draw/reset', 'reset'],
    ['random-draw/exhausted', 'exhausted'],
    ['random-draw/invalid-range', 'invalid-range'],
  ]);
  const baselineSnapshotFor = (stateId, viewport) => {
    if (standaloneDrawStateMap.has(stateId)) {
      const state = standaloneDrawBaseline.runs.current.states.find(({ state: candidate }) => candidate === standaloneDrawStateMap.get(stateId));
      return state?.records.find(({ viewport: candidate }) => candidate.width === viewport.width
        && candidate.height === viewport.height && (candidate.zoom ?? 1) === (viewport.zoom ?? 1));
    }
    const source = repairedStateIds.has(stateId) ? repairedBaseline : pristineBaseline;
    const state = source.states.find(({ id }) => id === stateId);
    if (stateId === 'timer/auction-award-presentation') {
      const candidate = state?.snapshots.find(({ viewport: baselineViewport }) => baselineViewport.width === viewport.width
        && baselineViewport.height === viewport.height && (baselineViewport.zoom ?? 1) === (viewport.zoom ?? 1));
      if (!candidate) return undefined;
      const scheduleEnd = candidate.text.indexOf('14:30');
      const footerStart = candidate.text.findIndex((value, index) => index > scheduleEnd && value === '\n    ');
      return {
        ...candidate,
        text: [
          ...candidate.text.slice(0, scheduleEnd + 1),
          '2', ' / ', '2', '1', '1', '번', '20 고마', '2', '2', '번', '25 고마',
          '2번', '연필', '25 고마', '확인',
          ...candidate.text.slice(footerStart),
        ],
      };
    }
    if (drawRuntimeRepairIds.has(stateId)) {
      const defaultState = pristineBaseline.states.find(({ id }) => id === 'timer/default');
      const candidate = defaultState?.snapshots.find(({ viewport: baselineViewport }) => baselineViewport.width === viewport.width
        && baselineViewport.height === viewport.height && (baselineViewport.zoom ?? 1) === (viewport.zoom ?? 1));
      return candidate ? { ...candidate, text: [candidate.text[0], '1', ...candidate.text.slice(1)] } : undefined;
    }
    return state?.snapshots.find(({ viewport: candidate }) => candidate.width === viewport.width
      && candidate.height === viewport.height && (candidate.zoom ?? 1) === (viewport.zoom ?? 1));
  };
  for (const state of result.states) {
    for (const snapshot of state.snapshots) {
      const baseline = baselineSnapshotFor(state.id, snapshot.viewport);
      const exactTextMatch = Boolean(baseline) && JSON.stringify(snapshot.text) === JSON.stringify(baseline.text);
      result.exactTextComparisons.push({ id: state.id, viewport: snapshot.viewport, exactTextMatch, source: standaloneDrawStateMap.has(state.id) ? 'repaired-todo7-standalone' : repairedStateIds.has(state.id) ? 'repaired-todo6' : 'pristine-todo1' });
      assert(exactTextMatch, `${state.id}: exact visible-text mismatch at ${snapshot.viewport.width}x${snapshot.viewport.height}`);
    }
  }
  result.authoritativeBaseline = {
    pristine: '.omo/evidence/task-1-apple-ui-refresh-plan/complete-visible-text-oracle.json',
    repaired: '.omo/evidence/task-6-apple-ui-refresh-plan/complete/authoritative-todo6-baseline.json',
    standaloneDraw: '.omo/evidence/task-7-apple-ui-refresh-plan/runtime.json',
    repairedStateIds: [...repairedStateIds],
    settledAwardRepair: 'The prior 320/390 award baseline captured transient step 1/2 while wider viewports captured settled step 2/2. The fixed fixture winner is bidder 2 at 25 고마, so the authoritative settled 1440 text inventory is applied to every viewport after explicit 2/2 readiness.',
  };
  assert(result.exactTextComparisons.length === result.expectedRecordCount, 'Exact visible-text comparison matrix incomplete');
  result.success = true;
} catch (error) {
