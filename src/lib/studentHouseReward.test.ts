...
test('학생 작품 열네 채의 제작자와 원본 이미지, 우표가 연결되어 있다', async () => {
  const houses = STUDENT_HOUSE_DESIGNS.filter(house => 'creatorStudentNumber' in house);
  assert.deepEqual(houses.map(house => house.creatorStudentNumber), [18, 10, 13, 6, 16, 4, 11, 12, 8, 20, 15, 7, 21, 23]);
  for (const house of houses) {
    ...
    assert.match(letter?.title ?? '', new RegExp(`${house.name}[이가] 팔렸어요!`));
    assert.doesNotMatch(letter?.title ?? '', /멋진 집/);
