...
test('class donation RPC deducts currency immediately while preserving auction reservations', async () => {
  ...
  const donationSql = sql.slice(sql.indexOf('create or replace function public.donate_to_class_goal'));
  ...
  assert.match(sql, /create table if not exists public\.class_donation_requests/i);
  assert.match(donationSql, /select result into v_result\s+from public\.class_donation_requests/i);
  assert.match(donationSql, /insert into public\.class_donation_requests/i);
  assert.match(
    donationSql,
  ...
  assert.match(donationSql, /grant execute on function public\.donate_to_class_goal\(integer, integer, text\) to service_role/);
});
