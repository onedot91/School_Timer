...
test('첨부한 46개 고마 스킨은 선택 가능한 카탈로그에 등록된다', () => {
  const addedSkinNames = [
    ...
    '복서 고마', '불 마법사 고마', '비눗방울 고마', '사또 고마', '수녀 고마',
...
test('고마 스킨 뽑기는 첫 1회 무료이고 다음 뽑기부터 50고마를 차감한다', () => {
  const first = applyStudentEconomyAction({
    state: null,
...
test('고마 스킨 뽑기는 미보유 스킨을 순서대로 지급하고 요청을 한 번만 처리한다', () => {
  ...
  assert.equal(second.wallet, 250);
...
test('10개 확률 구간 중 정확히 한 구간은 같은 50고마로 미보유 스킨 두 개를 지급한다', () => {
  let doubles = 0;
...
test('더블 캡슐도 중복 스킨을 제외하고 같은 요청을 다시 차감하거나 지급하지 않는다', () => {
  ...
  assert.equal(repeated.applied, false);
  assert.equal(repeated.wallet, 150);
...
test('미보유 스킨이 하나뿐이면 특별 구간에서도 한 개만 지급하고 모두 보유하면 차감을 막는다', () => {
  ...
  assert.equal(result.wallet, 150);
