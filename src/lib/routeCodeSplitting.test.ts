...
test('초기 번호 선택 화면은 교사와 학생 앱 번들을 필요할 때만 불러온다', () => {
  ...
  assert.match(rootAppSource, /void preloadEntryPage\(selectedEntryNumber\)[\s\S]*?void loadDeviceSession\(controller.signal\)/);
...
test('학생 첫 화면에 필요 없는 기능 UI는 기능을 열 때 불러온다', () => {
  ...
  assert.match(auctionPageSource, /<Suspense[\s\S]*?fallback=\{<AppLoadingScreen embedded \/>\}/);
});
