...
export const createBankMailboxLetters = ({
  ...
  if (action.type === 'open_deposit') {
    ...
    return [bankerLetter(
      ...
      selectBankerMessage(requestKey, [
        ...
        `꿀꿀, 예금한 ${action.amount} 고마를 돝돝이가 잘 보관하고 있어요. 만기는 고마를 찾기로 약속한 날이에요. ${maturityLabel}이 되면 ${payout} 고마를 돌려드릴게요!`,
      ]),
      createdAt,
    )];
  }
  ...
  if (action.type === 'repay') {
    return [bankerLetter(
      ...
      '△ 대출 상환 · 빌린 고마를 갚았어요',
      selectBankerMessage(requestKey, [
        `꿀꿀, ${action.amount} 고마를 잘 받았어요. 빌린 고마를 은행에 갚는 것을 상환이라고 해요. 돝돝이가 장부에도 정확하게 적어 두었꿀!`,
  ...
  if (action.type === 'transfer') {
    return [
      bankerLetter(
        ...
        '↗ 송금 완료 · 고마를 보냈어요',
        selectBankerMessage(requestKey, [
          `${action.recipientNumber}번에게 ${action.amount} 고마를 송금했어요. 송금은 다른 사람에게 고마를 보내는 일이에요. 돝돝이가 잘 도착하도록 안전하게 보냈꿀!`,
      ...
      bankerLetter(
        ...
        selectBankerMessage(requestKey, [
          ...
          `${studentNumber}번에게서 ${action.amount} 고마를 받았꿀. 다른 사람이 보낸 고마가 도착하는 것도 송금이에요. 돝돝이가 안전하게 넣어 두었어요.`,
