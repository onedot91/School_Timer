...
export function getVisibleStudentShopTabs(hasProfile: boolean, canAssignProfile = true): readonly ShopTab[] {
  if (hasProfile) return SHOP_TABS;
  if (!canAssignProfile) return ['characters', 'houses'];
  return PROFILE_ONLY_SHOP_TABS;
}
...
export default function StudentShopPage({
  ...
  const canAssignProfile = canAssignFailureProfile(studentNumber);
  const activeProfile = getAssignedFailureProfileImage(studentNumber, profileAssignments);
  const hasProfile = activeProfile !== null;
  const shopUnlocked = hasProfile || !canAssignProfile;
  const visibleTabs = getVisibleStudentShopTabs(hasProfile, canAssignProfile);
  const [tab, setTab] = useState<ShopTab>(() => visibleTabs[0]);
        ...
        repaired ? (
          <div id="student-shop-panel-houses" className="student-house-workshop" role="tabpanel" aria-labelledby="student-shop-tab-houses">
            <div className="student-house-market">
              ...
              {orderStudentHouseShopCatalog(getKoreanDateKey(), state.ownedHouseIds).map((house) => {
