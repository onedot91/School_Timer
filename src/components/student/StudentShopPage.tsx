...
export default function StudentShopPage({ state, catalog, isSaving, onAction }: StudentShopPageProps) {
  ...
  return (
    <section className="student-shop-hub" aria-labelledby="student-shop-title">
      ...
      <nav className="student-shop-tabs" aria-label="상점 종류">
        ...
        <button className={tab === 'characters' ? 'is-active' : ''} onClick={() => setTab('characters')}><Gamepad2 />캐릭터</button>
        ...
        <div className="student-shop-goods-grid">
          ...
          {repairItem ? (
            <article>
              ...
              <div><h3>{repairItem.name}</h3><span>{repaired ? '수리 완료' : '집 상점 열기'}</span></div>
              <button disabled={isSaving || repaired} onClick={() => void onAction({ type: 'buy_item', itemId: repairItem.id })}>{repaired ? '완료' : `${repairItem.price} 고마`}</button>
            </article>
          ) : null}
        </div>
      ) : null}
      ...
      {tab === 'characters' ? (
        <div className="student-character-arcade">
          ...
          <button className="student-character-draw-button" disabled={isSaving || state.ownedCharacterIds.length === STUDENT_CHARACTER_PRIZES.length} onClick={() => void onAction({ type: 'draw_character' })}><Sparkles />{STUDENT_CHARACTER_DRAW_PRICE} 고마 뽑기</button>
        </div>
      ) : null}
      ...
      {tab === 'houses' ? (
        ...
        ) : <div className="student-house-locked"><LockKeyhole /><h3>집 고치기 필요</h3><button onClick={() => setTab('items')}>물품에서 고치기</button></div>
      ) : null}
    </section>
  );
}
