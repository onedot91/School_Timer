import { parseTodayFriendCommonalities, type TodayFriendPayload } from '../../lib/todayFriend';
...
export default function TodayFriendSubmittedAnswer({ payload }: { readonly payload: TodayFriendPayload }) {
  const fields: readonly (readonly [string, string])[] = (() => {
    switch (payload.kind) {
      ...
      case 'commonality': {
        const items = parseTodayFriendCommonalities(payload.commonality);
        return items.some((item, index) => index > 0 && item.length > 0)
          ? items.map((item, index) => [`공통점 ${index + 1}`, item] as const)
          : [['대화로 찾은 공통점', payload.commonality]];
      }
