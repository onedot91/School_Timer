...
export default function TodayFriendMissionForm({
  ...
  return (
    <form className="today-friend-form" onSubmit={handleSubmit}>
      ...
      {mission.genre === 'commonality' ? (
        <label><span>대화로 찾은 공통점</span><textarea value={primaryText} onChange={(event) => setPrimaryText(event.target.value)} placeholder="취향, 경험, 관심사처럼 대화해야 알 수 있는 공통점을 적어요." maxLength={600} /><small>성별, 키, 외모처럼 눈으로 바로 보이는 특징은 적지 않아요.</small></label>
      ) : null}
      ...
      {mission.genre === 'compliment' ? (
        <label><span>친구에게 전할 구체적인 칭찬</span><textarea value={primaryText} onChange={(event) => setPrimaryText(event.target.value)} placeholder="언제, 어떤 행동을 보았고, 왜 좋았는지 적어요." maxLength={800} /><small>{primaryText.trim().length > 0 && primaryText.trim().length < 20 ? '어떤 행동이 왜 좋았는지 조금 더 자세히 적어 보세요.' : '그 행동이 나나 우리 반에 어떤 도움을 주었는지도 생각해 봐요.'}</small></label>
      ) : null}
      {mission.genre === 'emotion' ? (
        <>
          ...
          <small className="today-friend-privacy-note">말하고 싶지 않은 이야기는 억지로 묻지 않아요. 이 기록은 참여한 친구와 선생님만 볼 수 있어요.</small>
        </>
      ) : null}
      {formMessage ? <p className="today-friend-form-message" role="status">{formMessage}</p> : null}
      <div className="today-friend-form-actions">
        <button type="button" disabled={isSaving} onClick={() => { void save(false); }}>임시 저장</button>
        <button type="submit" disabled={isSaving}>{isSaving ? '저장 중…' : mission.submission?.status === 'revision_requested' ? '고쳐서 다시 제출' : '선생님께 제출'}</button>
      </div>
    </form>
  );
}
